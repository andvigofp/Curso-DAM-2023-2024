...
public class Barrera {
    ...
    synchronized public int entraCoches(int coche) throws InterruptedException {
        int plaza =0;
    ...
    synchronized public int entradaCamion(int camion) throws InterruptedException {
        int plaza=0;
        imprimirEstadoParking();
        //Mientras no haya plaza
        while ((plaza = buscarPlazasContiguas())==-1) {
            System.out.println("Camión " + camion + " esperando");
            wait();
        }
        plazas[plaza] = camion;
    ...
    synchronized public void salidaCamion(int plaza) {
        plazas[plaza] =0;
        plazas[plaza + 1] =0;
        libres +=2;
        notifyAll();
